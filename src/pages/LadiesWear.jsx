...
const LadiesWear = () => {
  const { ladiesProducts } = useContext(AdminContext);
  const [filter, setFilter] = useState("All");

  const filteredProducts = ladiesProducts.filter((product) => {
    ...
    <main className="min-h-screen">
      ...
      <section className="relative bg-[#071952] text-white py-20 md:py-32 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 text-center z-10">
          ...
          <p className="text-xl md:text-2xl text-gray-300 font-medium tracking-wide">
            Elegant & Fashionable Collection for Every Woman
          </p>
        </div>
      </section>
      ...
      <section className="bg-white border-b border-gray-200 py-6 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-wrap gap-3 justify-center">
            ...
            <button onClick={() => setFilter("Sarees")} className={getBtnClass("Sarees")}>Sarees</button>
            <button onClick={() => setFilter("Kurtis")} className={getBtnClass("Kurtis")}>Kurtis</button>
            <button onClick={() => setFilter("Lehengas")} className={getBtnClass("Lehengas")}>Lehengas</button>
            <button onClick={() => setFilter("Gowns")} className={getBtnClass("Gowns")}>Gowns</button>
      ...
      <section className="bg-gradient-to-r from-[#D4A017] to-[#C39428] text-[#071952] py-12 md:py-16">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-4 uppercase">Celebrate Your Beauty</h2>
          <p className="text-lg mb-6">Discover our exclusive ladies wear collection</p>
          <a
